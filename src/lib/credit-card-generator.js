// Based on https://github.com/VRMink/credit-card-generator/blob/master/index.js
 ...
 * @return {String}
 */
export function generateCreditCard(CardScheme, howMany = 1) {
  // Try to get configs to the selected Scheme
  if (CreditCardSchemes[CardScheme] != 'undefined') {
    return credit_card_number(
      CreditCardSchemes[CardScheme].prefixList,
      CreditCardSchemes[CardScheme].digitCount,
      howMany
    );
  } else {
    // Defaults to MasterCard
    return credit_card_number(
      CreditCardSchemes['MasterCard'].prefixList,
      CreditCardSchemes['MasterCard'].digitCount,
      howMany
    );
  }
}
